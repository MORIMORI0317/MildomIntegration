...
public class ItemGift extends ItemFood {
    ...
    protected void onFoodEaten(ItemStack stack, World worldIn, EntityPlayer player) {
        if (!worldIn.isRemote) {
            Gift gift = Gift.getGiftByItemID(stack.getItemDamage());
            ...
            int price = Gift.getGiftByItemID(stack.getItemDamage()).getPrice();
    ...
    public void addInformation(ItemStack stack, @Nullable World worldIn, List<String> tooltip, ITooltipFlag flagIn) {
        if (Emojicord.isLoaded())
            tooltip.add(Emojicord.getPriceEmoji() + " " + Gift.getGiftByItemID(stack.getItemDamage()).getPrice());
        else
            tooltip.add(I18n.format("item.gift.price", Gift.getGiftByItemID(stack.getItemDamage()).getPrice()));
    }
}
